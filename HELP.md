...
单例模式（enum包）：为了把数据全局写入同一份内存（队列+链表+HashMap），使用了单例，保证了系统中唯一性

策略模式(strategy包)：基于接口编程，对系统功能做抽象，提取出公共部分，然后做不同的实现，这里为了系统扩展开放，
 1.快递员的调度策略
    a.Match
    b.FIFO
 2.hash算法策略实现（调度策略中Match策略给订单分配快递员） --预留多种实现方式
 3.限流策略--预留多种实现方式
    a.计数器限流
    b.滑动窗口限流--sentinal之类
    c.漏斗桶限流
...
监听者模式（event+listener包）---异步解耦。基于Event机制（spring的，也可以基于google的Event-Bus框架）
    a.下单成功监听事件
    b.餐厅接收到订单事件
    c.餐厅制作完订单事件
桥接模式（bridge包）---异常订单告警
    ...
    最后通过组合的方式，拼在一起
    实现了异常登录和告警方式的灵活排列组合
代理模式（aop包）--开发了自定义注解(底层基于java反射)
    a.日志打印
    b.限流功能

-----------------并发-----------------
做了限流控制
ConcurrentHashMap--加锁map
synchronized --加锁
AtomicInter 原子类 保证id生成的唯一性
Volital --可见性 保证线程安全 
队列用到LinkedBlockingQueue 基于Valitia和CAS 自旋 保证线程安全
PriorityBlockingQueue  ----也是线程安全的
DelayQueue 由于其堆的数据结构存储，不会出现多个线程操作一个数据，所有本身也是线性安全的
单例加锁检查
单例（本身的唯一性避免了线程安全问题）
...
hash:HashMap
线程池：SchedualThreadPoolExecutor
